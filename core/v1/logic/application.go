...
func (a applicationService) CreateGithubWebHookAndStoreApplication(token string, app v1.Application) {
	...
	gitWebhook, err := NewGithubService(nil, a.client).CreateRepositoryWebhook(usernameOrorgName, repoName, token, app.CompanyId, app.MetaData.Id, app.Secret)
	if err != nil {
...
func (a applicationService) CreateBitbucketWebHookAndStoreApplication(token string, app v1.Application) {
	...
	gitWebhook, err := NewBitBucketService(nil, a.client).CreateRepositoryWebhook(usernameOrorgName, repoName, token, app.CompanyId, app.MetaData.Id, app.Secret)
	if err != nil {
		log.Println("ERROR while creating webhook for application: ", err.Error())
	}
	app.Webhook = gitWebhook
...
func (a applicationService) EnableBitbucketWebhookAndUpdateApplication(companyId, repoId, appId, url, token string) error {
	...
	webhook, err := NewBitBucketService(nil, a.client).CreateRepositoryWebhook(username, repositoryName, token, companyId, appId, app.Secret)
	if err != nil {
		return err
	}
	app.Webhook = webhook
...
func (a applicationService) EnableGithubWebhookAndUpdateApplication(companyId, repoId, appId, url, token string) error {
	...
	app := a.repo.GetByIdAndRepoId(companyId, repoId, appId)
	webhook, err := NewGithubService(nil, a.client).CreateRepositoryWebhook(username, repositoryName, token, companyId, appId, app.Secret)
	if err != nil {
		return err
	}
	app.Webhook = webhook
